...
domain = "beta.the-eye.eu/public/Books"  # <- your domain to be crawled
full_url = "https://beta.the-eye.eu/public/Books/"
...
def crawl(url):
    ...
    if not os.path.exists(f"pdfs/{local_domain}/"):
        os.mkdir(f"pdfs/{local_domain}/")
    ...
    while queue:
        ...
        # check if the URL ends with ".pdf" and download it if it does
        if url.lower().endswith(".pdf"):
            pdf_name = url.split("/")[-1]
            pdf_save_path = os.path.join("pdfs", local_domain, pdf_name)
